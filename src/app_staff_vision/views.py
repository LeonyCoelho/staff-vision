...
from app_staff_vision.models import Establishment, Sector, Sub_Sector, Position, Shift, Global_Settings, Status, Theme, Worker, Worker_History, CustomUser, Dashboard_Presets, Preset_Settings, Preset_Header
...
def apply_defauts(request):
    default_theme = Theme(
        id = 1,
        theme_name = 'default',
        theme_url = 'css/default.css'
    )
    default_theme.save()
    fluent_theme = Theme(
        id = '2',
        theme_name = 'fluent',
        theme_url = 'css/fluent.css'
    )
    fluent_theme.save()
...
def remove_logo(request):
    last_global_settings  = Global_Settings.objects.latest('id')
    last_global_settings.logo_image = None
    last_global_settings.save()
    return redirect('settings')
...
def remove_bg(request):
    last_global_settings  = Global_Settings.objects.latest('id')
    last_global_settings.bg_image = None
    last_global_settings.save()
    return redirect('settings')
